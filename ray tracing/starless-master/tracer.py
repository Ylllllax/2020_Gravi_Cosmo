...
for arg in sys.argv[1:]:   # 终端输入参数读取
    if arg == '-d':
        LOFI = True
        continue
    if (arg == '--no-graph'):
...
#GRAPH
if DRAWGRAPH:   # 画出示意图 画出光子圆轨道 相机位置 史瓦西半径
...
if not DISABLE_SHUFFLING:
    np.random.shuffle(pixelindices)   # 像素点打乱顺序
chunks = np.array_split(pixelindices,numPixels/CHUNKSIZE + 1)   # 把像素分成3份（3个chunk）每个6400
...
def raytrace_schedule(i,schedule,total_shared,q): # this is the function running on each thread
    ...
    for chunk in schedule:
        #if killers[i]:
        #    break
        # chunk 为每一列 schedule 里全部 chunk 数据
        chnkcounters[i]+=1

        #number of chunk pixels
        numChunk = chunk.shape[0]  # 6400

        #useful constant arrays 
        ones = np.ones((numChunk))
        ones3 = np.ones((numChunk,3))
        UPFIELD = np.outer(ones,np.array([0.,1.,0.])) # outer 外积 结果为6400行[0,1,0]
        BLACK = np.outer(ones,np.array([0.,0.,0.]))

        #arrays of integer pixel coordinates
        x = chunk % RESOLUTION[0]   # 返回余数， RESOLUTION[0]=160，即x轴位置
        ...
        view = np.einsum('jk,ik->ij',viewMatrix,view)  # 点积
